...
public interface InterPizzaiolo {
   ...
   *         ingrédient du même nom, -3 si le prix est invalide (inférieur ou égal
   *         à 0)
   */
   ...
   * @return 0 si tout s'est bien passé, -1 si le nom de l'ingrédient n'est pas
   *         valide (chaine vide ou <code>null</code>), -2 si le prix est invalide
   *         (inférieur ou égal à 0), -3 si aucun ingrédient de ce nom n'existe
   */
   ...
   * pizza. Par exemple, du jambon ou du boeuf haché ne peut pas être utilisé dans
   ...
   * @return <code>true</code> si l'ingrédient a bien été ajouté comme exclu pour
   *         le type de pizza, <code>false</code> en cas de problème (l'ingrédient
   *         n'existait pas...) et dans ce cas, aucune exclusion n'est ajoutée
   */
   ...
   * nom. Une fois créée, la pizza est visible par les clients et disponible à la
   * vente. Toutes les opérations de cette interface qui prennent en paramètre une
   * pizza ne s'appliquent que sur des pizzas créées par cette méthode.
   *
   * @param nom  le nom de la nouvelle pizza
   * @param type le type de la pizza
   * @return la pizza créée ou <code>null</code> en cas de problème (nom invalide
   *         ou pizza du même nom existant déjà)
   */
  Pizza creerPizza(String nom, TypePizza type);

  /**
   * Ajoute un ingrédient à une pizza. Ne fait rien si l'ingrédient avait déjà été
   * ajouté à la pizza.
   *
  ...
  /**
   * Vérifie que des ingrédients d'une pizza ne sont pas interdits pour le type de
   * la pizza. Cette vérification est utile si on a ajouté des interdictions
   ...
   *         vide si tous les intégrients sont valides) ou <code>null</code> si la
   *         pizza n'est pas valide
   */
  Set<String> verifierIngredientsPizza(Pizza pizza);

  /**
   * Ajoute une photo à une pizza. Remplace la photo précédente si une photo était
   * déjà associée à la pizza.
   *
  ...
  /**
   * Modifie le prix de vente d'une pizza. Le prix doit être supérieur ou égal au
   * prix minimal de la pizza.
   *
   ...
   *
   * @return l'ensemble des pizzas (l'ensemble est vide si aucune pizza n'a encore
   *         été créée)
   */
  Set<Pizza> getPizzas();
   ...
   *
   * @return l'ensemble des clients (l'ensemble est vide s'il n'y a aucun client)
   */
   ...
   *
   * @return l'ensemble ordonné des commandes qui ont été traitées (l'ensemble est
   *         vide si aucune commande n'a été traitée)
   */
  List<Commande> commandesDejaTraitees();

  /**
   * Retourne l'ensemble des commandes des clients non encore traitées. Elles sont
   * classées de la plus ancienne à la plus récente. Une fois que ces commandes
   * ont été lues en appelant cette méthode, on les considère comme traitées.
   *
   ...
   *         client n'a pas passé de commandes) ou <code>null</code> si le client
   *         n'est pas valide
   */
  ...
  /**
   * Calcule le bénéfice d'une commande. Pour cela, on fait la somme des bénéfices
   * de chacune des pizzas de la commande.
   *
   * @param commande la commande dont on veut calculer le bénéfice
   * @return le bénéfice de la commande (ou -1 si la commande n'était pas valide)
   */
  ...
  /**
   * Calcule le nombre total de pizzas commandées par chaque client (en se basant
   * sur les commandes déjà traitées).
   *
   ...
   * basant sur les commandes déjà traitées). La première pizza de la liste est la
   * pizza la plus commandée, la dernière de la liste est la pizza la moins
   * commandée.
   *
