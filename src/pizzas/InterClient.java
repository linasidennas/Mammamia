...
public interface InterClient {
   ...
   *         s'est bien déroulée, -1 si l'email est déjà utilisé, -2 si l'email ou
   *         le mot de passe est vide, -3 si les informations personnelles ne sont
   *         pas bien précisées, -4 si l'adresse email n'est pas bien formée
   */
  ...
  /**
   * Crée une nouvelle commande pour le client actuellement connecté. On ajoutera
   * ensuite des pizzas à la commande avant de la valider.
   *
   ...
   *                              commande : la commande n'est pas une commande en
   *                              cours, la commande n'a pas été créée par le
   *                              client connecté, la pizza n'existe pas...
   */
  void ajouterPizza(
       Pizza pizza, int nombre, Commande cmd) throws NonConnecteException, CommandeException;

  /**
   * Valide une commande en cours. Une fois cela fait, la commande est visible par
   * le pizzaïlo et le client ne peut plus la modifier. La commande apparaîtra
   * dans la liste des commandes passées du client.
   *
   ...
   *                              client connecté...
   */
  void validerCommande(Commande cmd) throws NonConnecteException, CommandeException;

  /**
   * Annule une commande en cours. Une fois cela fait, la commande n'existe plus.
   *
   ...
   *                              client connecté...
   */
  void annulerCommande(Commande cmd) throws NonConnecteException, CommandeException;

  /**
   * Renvoie la liste des commandes actuellement en cours (créés mais non encore
   * validées) pour le client connecté.
   *
   * @return les commandes en cours du client connecté, ordonnées par création (la
   *         plus ancienne en premier, la plus récente en dernier)
   * @throws NonConnecteException si aucun client n'est connecté
   */
   ...
   *
   * @return les commandes passées du client connecté, ordonnées par création (la
   *         plus ancienne en premier, la plus récente en dernier)
   * @throws NonConnecteException si aucun client n'est connecté
   */
   ...
   *
   * @return l'ensemble des pizzas (l'ensemble est vide si aucune pizza n'existe)
   */
  Set<Pizza> getPizzas();
  ...
  /**
   * Ajoute un filtre sur les ingrédients. Les pizzas conservées contiennent tous
   * les ingrédients de la liste. Si un ingrédient n'est pas valide, il est
   * ignoré.
   *
  ...
  /**
   * Ajoute une évaluation à une pizza de la part du client connecté. L'évaluation
   * consiste à mettre une note (une valeur entière entre 0 et 5) avec
   * éventuellement un commentaire textuel. Le client connecté doit avoir déjà
   * commandé la pizza (dans une commande validée) pour pouvoir faire un
  ...
  boolean ajouterEvaluation(
       Pizza pizza, int note, String commentaire) throws NonConnecteException, CommandeException;
